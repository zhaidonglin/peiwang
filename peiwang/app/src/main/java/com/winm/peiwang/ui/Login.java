...
import com.winm.peiwang.domain.LoginData;
import com.winm.peiwang.domain.User;
import com.winm.peiwang.utils.Parser;
...
public class Login extends KJActivity {
    ...
    // wdiget
    @BindView(id = com.winm.peiwang.R.id.login_img_logo)
    private ImageView mImgLogo;
    @BindView(id = com.winm.peiwang.R.id.login_layout_input)
    private RelativeLayout mLayoutInput;
    @BindView(id = com.winm.peiwang.R.id.login_et_uid)
    private EditText mEtUid;
    @BindView(id = com.winm.peiwang.R.id.login_img_delete, click = true)
    private ImageView mImgDel;
    @BindView(id = com.winm.peiwang.R.id.login_et_pwd)
    private EditText mEtPwd;
    @BindView(id = com.winm.peiwang.R.id.login_btn_login, click = true)
    private Button mBtnLogin;
    @BindView(id = com.winm.peiwang.R.id.login_img_head)
    ...
    public void setRootView() {
        setContentView(com.winm.peiwang.R.layout.aty_login);
    }
    ...
    private void screenAdapter() {
        ...
        int headH = (int) getResources().getDimension(
                com.winm.peiwang.R.dimen.splash_head_height);
        // 头像适配
    ...
    public void widgetClick(View v) {
        super.widgetClick(v);
        switch (v.getId()) {
            case com.winm.peiwang.R.id.login_btn_login:
                doLogin();
                break;
            case com.winm.peiwang.R.id.login_img_delete:
                mEtUid.setText(null);
                break;
        }
    }
    ...
    private boolean inputCheck() {
        if (StringUtils.isEmpty(mEtUid.getText().toString())) {
            ViewInject.toast(getString(com.winm.peiwang.R.string.account_not_empty));
            return false;
        }
        if (StringUtils.isEmpty(mEtPwd.getText().toString())) {
            ViewInject.toast(getString(com.winm.peiwang.R.string.password_not_empty));
            return false;
        }
        return true;
    }
    ...
    private void doLogin() {
        ...
        if(mEtUid.getText().toString().compareTo("1") == 0 &&
                mEtPwd.getText().toString().compareTo("1") == 0){

            try {
                java.util.Date nowdate = new java.util.Date();
                String myString = "2016-08-30 09:00";
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm", Locale.CHINA);
                Date d = sdf.parse(myString);

                boolean flag = d.before(nowdate);
                if (flag) {
                    ViewInject.toast("试用版已过期，请联系客服");
                }
                else {
                    ViewInject.toast("登录成功");
                    //页面跳转
                    Intent jumpIntent = new Intent();
                    jumpIntent.setClass(aty, EventsActivity.class);
