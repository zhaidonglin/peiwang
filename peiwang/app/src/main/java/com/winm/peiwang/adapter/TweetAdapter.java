...
import com.winm.peiwang.domain.Tweet;
import com.winm.peiwang.ui.widget.CollapsibleTextView;
...
public class TweetAdapter extends KJAdapter<Tweet> {
    ...
    public void convert(AdapterHolder helper, Tweet item, boolean isScrolling) {
        ...
        if (isScrolling) {
            kjb.displayCacheOrDefult(helper.getView(com.winm.peiwang.R.id.msg_item_img_head),
                    headUrl, com.winm.peiwang.R.drawable.default_head);
        } else {
            kjb.display(helper.getView(com.winm.peiwang.R.id.msg_item_img_head), headUrl, 135,
                    135, com.winm.peiwang.R.drawable.default_head);
        }
        helper.setText(com.winm.peiwang.R.id.msg_item_text_uname, item.getAuthor());
        CollapsibleTextView content = helper
                .getView(com.winm.peiwang.R.id.msg_item_text_content);
        content.setText(InputHelper.displayEmoji(content.getResources(),
                item.getBody(), "[", "]"));

        helper.setText(com.winm.peiwang.R.id.msg_item_text_time,
                StringUtils.friendlyTime(item.getPubDate()));

        View image = helper.getView(com.winm.peiwang.R.id.msg_item_img);
        if (StringUtils.isEmpty(item.getImgBig())) {
            ...
            if (isScrolling) {
                kjb.displayCacheOrDefult(image, item.getImgBig(),
                        com.winm.peiwang.R.drawable.pic_bg);
            } else {
